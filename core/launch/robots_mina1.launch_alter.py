...
###
# THIS FUNCTION GENERATES A "LaunchDescription" OBJECT THAT DESCRIBES HOW TO LAUNCH THE GAZEBO SIMULATION
###
def generate_launch_description():
    ld = LaunchDescription()
    
    ###
    # INSERT NAME AND POSES OF THE ROBOTS HERE
    ###
    robots = [
        {'name': 'tb_1', 'model':'burger','x_pose': '1.6', 'y_pose': '-0.2', 'z_pose': 0.01},    #OK
        #{'name': 'tb_2', 'model':'burger','x_pose': '0.65', 'y_pose': '31.5', 'z_pose': 0.01},   #OK
        #{'name': 'tb_3', 'model':'burger','x_pose': '0.5', 'y_pose': '63.15', 'z_pose': 0.01},   #OK
        {'name': 'tw_1', 'model':'waffle', 'x_pose': '1.2', 'y_pose': '15.45', 'z_pose': 0.01},  #OK
        #{'name': 'tw_2', 'model':'waffle', 'x_pose': '1.05', 'y_pose': '-15.77', 'z_pose': 0.01},#OK
        #{'name': 'tw_3', 'model':'waffle','x_pose': '1.05', 'y_pose': '47.26', 'z_pose': 0.01}   #OK
        ]
    ...
    ###
    # FOLDER CONTAINING THE MODELS OF THE ROBOTS
    ###
    ...
    map_server=Node(package='nav2_map_server',
        ...
        name='map_server',
        output='screen',
        parameters=[{'yaml_filename': os.path.join(get_package_share_directory('thesis_simulation'), 'maps', 'mina_with_stones_map.yaml'),
                     },],
    ...
    for robot in robots:
        ...
        urdf = os.path.join(
        turtlebot3_multi_robot, 'urdf', 'turtlebot3_' + robot['model'] + '.urdf'
        )
        ...
        # Create spawn call
        #IT TAKES THE .sdf FILES CONTAINING THE MODELS OF THE ROBOTS
    ...
    for robot in robots:
        ...
        X = float(robot['x_pose']) + 137.0
        robot['x_pose'] = str(X)
         
        Y = float(robot['y_pose']) + -110.5
        robot['y_pose'] = str(Y)
